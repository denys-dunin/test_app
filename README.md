...
You're on macOS (usually) with a Python that doesn't have OS-level trust roots
— common with the python.org installer. The script already tries to fix this
by using `certifi`'s CA bundle, so the first thing to try is:

```bash
pip install -r requirements.txt   # pulls in certifi
```

If you can't or don't want to install `certifi`, run the one-shot installer
that ships with python.org Python:

```bash
/Applications/Python\ 3.12/Install\ Certificates.command   # adjust version
```

Or point at an existing CA bundle for one run:

```bash
SSL_CERT_FILE=/etc/ssl/cert.pem python3 weather_alerts.py
```
